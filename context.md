...
| **Scenario document** | Сценарий в YAML | The scenario as YAML, written in the same keys the API takes. The editor shows it in a text area — edit or paste it there and apply it back — and downloads it as a file to keep with the rest of a game's material or carry into another game. Media never travels in it, only file guids and names; the files are uploaded to the game separately and matched to the hints by name on import. | `constructor/scenario-yaml.ts`, `constructor/game-editor.component` |
...
| Term | Русский | Meaning | Where |
| --- | --- | --- | --- |
| **Season** | Сезон | One calendar year's plan of games, as a list of dates. It exists only once published — there is no draft on the server; composing one lives in component state. | `season/`, `GET /seasons/{year}` |
| **Slot** | Дата игры (в разговоре — просто «дата») | One planned date in a season, before there is a game to put in it. Date-only; free or taken; may be linked to exactly one game. Никаких «слотов» в русских строках — только «дата». A day travels as a plain `YYYY-MM-DD` string and is never parsed into a `Date` — that would shift it by a timezone. | `season/season.models.ts`, `season-calendar.ts` |
| **Taking a slot** | Взять дату | An author claiming a date, declaring whether the game will be authored by them or by their team. It records intent, not exclusive access. Naming a *team* needs its captain; putting somebody else's player or team on a date is an admin-panel action, never part of the ordinary schedule. | `season-slot-dialog.component`, `POST /seasons/{year}/slots/{id}/take` |
| **Slot owner** | Владелец даты | The author who took it — who means to make the game there. Any author may still move, release or delete the date; the season gives nobody a lock and needs no admin role. | `season/season.models.ts` |
...
| **Schedule publication** | Публикация расписания | Confirming a composed season in one request. Not *публикация результатов* — that is a different act on a different aggregate. A 409 means somebody published that year first: the local list is dropped and the published season is shown. | `POST /seasons` |
| **Composed list** | Собираемое расписание | The working list of a season that is not published yet. Per-device like the theme: component state mirrored to `localStorage`, never sent anywhere until «Опубликовать». | `season/season-compose.ts` |
...
The calendar is at `/season` and `/season/:year`, readable by anyone. A tap on
a day only ever opens a window — it never writes — and the page renders the
same for an engine admin as for everybody else: what an admin may do that an
author may not lives in the admin panel.
