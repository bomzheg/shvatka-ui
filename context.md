...
| **Spoiler** | Спойлер | A photo hint part the author chose to deliver covered: the player sees a blurred image and uncovers it with a tap, exactly as in Telegram. The caption stays visible. Photos only; absent or `null` means no spoiler. | `HintPart.has_spoiler`, `hint.part/`, `constructor/hint-editor.component` |
