...
| **Scenario document** | Сценарий файлом (YAML) | The scenario as a YAML file, written in the same keys the API takes: exported from the editor, edited by hand or kept with the rest of a game's material, imported back — into another game too. Media never travels in it, only file guids and names; the files are uploaded to the game separately and matched to the hints by name on import. | `constructor/scenario-yaml.ts`, `constructor/game-editor.component` |
