...
export class HintEditorComponent {
  ...
  /**
   * How the markup refers to this file. Telegram resolves embedded media
   * through a `tg://<kind>?id=<id>` link, and knows three kinds only — a
   * gif travels as a video, a voice message as audio.
   */
  richMediaReference(media: RichMedia): string {
    const link = `tg://${this.richMediaKind(media)}?id=${media.id}`;
    if (this.richFormat === RichFormat.markdown) {
      return `![](${link})`;
    }
    switch (this.richMediaKind(media)) {
      case "video":
  ...
  /** The link scheme telegram addresses this file by, from its content type. */
  private richMediaKind(media: RichMedia): "photo" | "video" | "audio" {
